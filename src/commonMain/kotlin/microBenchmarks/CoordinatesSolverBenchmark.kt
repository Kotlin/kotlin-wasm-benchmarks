...
class CoordinatesSolverBenchmark {
    ...
    class Solver(private val input: Input) {
        private lateinit var objects: List<Coordinate>

        private var width: Int = 0
        private var height: Int = 0
        private lateinit var maze_end: Coordinate

        private var counter: Long = 0

        fun setup() {
            val newObjects = ArrayList<Coordinate>()
            for (f in input.labyrinth.fields) {
                if (f.hasObject()) {
                    newObjects.add(Coordinate(f.x, f.y))
                }
            }
            objects = newObjects
